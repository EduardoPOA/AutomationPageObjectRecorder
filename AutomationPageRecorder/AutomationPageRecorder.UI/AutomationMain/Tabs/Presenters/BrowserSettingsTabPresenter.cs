...
namespace AutomationPageRecorder.UI
{
    public class BrowserSettingsTabPresenter : IPresenter<BrowserSettingsTabView>
    {
        ...
        public void InitWithView(BrowserSettingsTabView view)
        {
            ...
            view.SetStatus("Desativado");
        }

        private void InitControls()
        {
            var driverIsRunning = AutomationBrowser.IsWorking;
            if (driverIsRunning)
            {
                view.SetStatus("Ativado");
        ...
        public void StartDriver(WebDriverOptions browserOptions, bool shouldMaximizeBrowserWindow)
        {
            ...
            bool isSuccessful = UIActions.PerformSlowOperation(
                                ...
                                () =>
                                {
                                    AutomationBrowser.Initialize(browserOptions);
                                    wasBrowserStarted = true;

                                    if (shouldMaximizeBrowserWindow)
                                    {
                                        AutomationBrowser.Maximize();
                                    }
        ...
        public void StopDriver()
        {
            ...
            UIActions.PerformSlowOperation(
                        ...
                        () =>
                        {
                            Presenters.AutomationMainPresenter.StopVisualSearch();
        ...
        {
            var remoteDriver = (RemoteWebDriver)AutomationBrowser.GetDriver();
        ...
        public bool TestRemoteHub(string url)
        {
            ...
            try
            {
                AutomationBrowser.TestRemoteHub(url);
            }
            catch (Exception e)
            {
                isOk = false;
                result = "Falhou: " + e.Message;
            }
