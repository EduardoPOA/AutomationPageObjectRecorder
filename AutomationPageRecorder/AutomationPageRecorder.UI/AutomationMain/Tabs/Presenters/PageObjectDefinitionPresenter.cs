...
namespace AutomationPageRecorder.UI
{
    public class PageObjectDefinitionPresenter : IPresenter<PageObjectDefinitionView>
    {
        ...
        internal void removeElementFromObjectPage(WebElementDefinition element)
        {
            if (_isEditingExistingNode)
            {
                DialogResult dr = MessageBox.Show("Você deseja remover o elemento que é nomeado como " + _currentEditingNode.Text + "?", "Attention", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        internal AutomationPageObject GetWebElementDefinitionFromTree()
        {
        ...
        private void SavePageObjectToFile(string targetFullPath)
        {
            ...
            {
                var serializer = new XmlSerializer(typeof(AutomationPageObject));
        ...
        internal void LoadPageObject(string pageObjectFileName)
        {
            ...
            AutomationPageObject pageObject = null;
            try
            {
        ...
        private AutomationPageObject LoadPageObjectFromFile(string pageObjectFileName)
        {
            AutomationPageObject definitions = null;

            using (FileStream stream = File.OpenRead(pageObjectFileName))
            {
                var serializer = new XmlSerializer(typeof(AutomationPageObject));
                definitions = (AutomationPageObject)serializer.Deserialize(stream);
            }
            return definitions;
        }
