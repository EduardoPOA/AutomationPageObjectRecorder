...
namespace AutomationPageRecorder.UI
{
    public class PageObjectSourceCodePresenter : IPresenter<PageObjectSourceCodeView>
    {
        ...
        private const string LINE_MUST_STARTS_WITH = "<element key";
        private const string RGX_PATTERN_SEARCH_FULL_KEY = "(?<=key=\")(.*?)(?=\\\")";//from key=" to " /ex ...key="Edu.aaa"... => Edu.aaa
        private const string RGX_PATTERN_SEARCH_KEY = "(?<=key=\")(.*?)(?=\\.)";//from key=" to . /ex ...key="Edu.aaa... => Edu
        ...
        internal void MachineLearningGenerateCode()
        {
            string nameFeature = view.GetCodeStartWithKeyXml();
            GenerateTemplateProject(nameFeature);
        }

        internal void GenerateTemplateProject(string nameFeature)
        {
            //cypresss
            string folderE2E = "cypress\\e2e";
       ...
       .FirstOrDefault(dir => dir.EndsWith(Path.Combine(folderE2E), StringComparison.OrdinalIgnoreCase));
                getE2E = directory2e2;
       ...
       .FirstOrDefault(dir => dir.EndsWith(Path.Combine(folderSupportPages), StringComparison.OrdinalIgnoreCase));
                ...
                foreach (string file in directoriesFiles.Distinct().Concat(directories))
                {
                    ...
                    if (getE2E != null)
                    {
                        if (getE2E.EndsWith("e2e"))
                        {
                            ...
                            {
                                string putUrl = "https://www.??.com.br/";
                    ...
                    if (file.EndsWith("_Pages"))
                    {
                        ...
                        if (!File.Exists(getPageCs))
                        {
                            ...
                            using (FileStream fs = File.Create(getPageCs)) ;
                            using (StreamWriter wt = new StreamWriter(getPageCs))
                            {
                    ...
                    if (file.EndsWith("_Steps"))
                    {
                        ...
                        string namePageName = Path.GetFileName(getPageCs);
        ...
        private void addPlugins()
        {
            ...
            foreach (XmlNode no in nosItemGroup)
            {
                ...
                foreach (string item in plugins)
                {
                    ...
                    version.Value = (item == "ExtentReports") ? "4.1.0" : (item == "NUnit") ? "3.14.0" : "*";
        ...
        internal void ReadTemplateCshtml()
        {
            ...
            write.Write(view.GetCodeFromTextSourceCshtml());
            write.Close();
            ...
            list.SelectMany(x => x.Replace("bool \"", "bool ").Replace("\"()", "()"));
            File.WriteAllLines(getTextMethodsFromCshtml, list);
            foreach (string item in list)
            {
                if (item.StartsWith("public") || item.StartsWith(" public"))
                {
                    string itemReplace = item.Replace("string ", "").Replace("int ", "").Replace("\"", "");
                ...
                foreach (string beforeItem in File.ReadLines(Path.Combine(GetAssemblyDirectory(), "_stepGenerator.txt")))
                {
                    ...
                    try
                    {
                        ...
                        using (StreamReader readPage = new StreamReader(getPageCs))
                        {
                            ...
                            textScript = textScript.Remove(textScript.LastIndexOf('}') + 1).Replace("bool \"", "bool ").Replace("\"()", "()").Replace("\"(string value)", "(string value)");
        ...
        private void replaceDoubleQuotesBaseValue()
        {
