...
using AutomationPageRecorder.WebDriver.OpenQA.Selenium.Support.PageObjects;


namespace AutomationPageRecorder.WebDriver
{
    public static class AutomationBrowser
    {
        ...
        public static IWebElement[] GetAllFrameElements()
        {
            lock (lockObject)
            {
                var driver = AutomationBrowser.GetDriver();
        ...
        public static List<BrowserPageFrame> GetPageFramesTree(BrowserPageFrame parent)
        {
            ...
            lock (lockObject)
            {
                ...
                for (var i = 0; i < allFramesOnThePage.Length; i++)
                {
                    ...
                    AutomationBrowser.GoToFrame(parent, ref frameStack);
        ...
        private static void GoToFrame(BrowserPageFrame frame, ref List<BrowserPageFrame> frameStack)
        {
            ...
            {
                AutomationBrowser.GetDriver().SwitchTo().DefaultContent();
                frameStack.Reverse();
                foreach (var frameItem in frameStack)
                {
                    AutomationBrowser.GetDriver().SwitchTo().Frame(frameItem.Index);
                }
                return;
            }
            else
        ...
        { 
            get {
                    lock (lockObject)
                    {
                        return AutomationBrowser.GetDriver().Url;
                    }
                } 
            set {
                    lock (lockObject)
                    {
                        AutomationBrowser.GetDriver().Url = value;
                    }
                }
        }
        ...
        public static void Maximize()
        {
            lock (lockObject)
            {
                AutomationBrowser.GetDriver().Manage().Window.Maximize();
            }
        }
        ...
        public static By ConvertLocatorSearchMethodToBy(LocatorSearchMethod searchMethod, string locator)
        {
            ...
            switch (searchMethod)
            {
                ...
                case LocatorSearchMethod.link:
                    by = By.LinkText(locator);
                    break;

                case LocatorSearchMethod.partiallink:
